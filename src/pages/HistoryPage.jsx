...
export default function HistoryPage() {
  ...
  const fetchSummaries = () => {
    fetch(`${BASE_URL}/summaries/${userId}`)
      .then((res) => res.json())
      .then((data) => {
        setSummaries(data);
        setLoading(false);
  ...
  const handleDelete = async (docId) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/summary/${userId}/${docId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.status === "deleted") {
        setSummaries((prev) => prev.filter((item) => item.id !== docId));
      }
    } catch (err) {
  ...
  if (loading)
    return <p className="p-6 text-gray-600 animate-fade-in">Loading summaries...</p>;
